...
export async function getActiveUserBan(userId: string): Promise<BanInfo | null> {
  const [row] = await db
    .select({
      id: ban.id,
      reason: ban.reason,
  ...
  if (!row) {
    ...
    await db.update(user).set({ isBanned: false, banExpiresAt: null }).where(eq(user.id, userId))
    return null
  }
...
export async function getActiveIpBan(ip: string | null): Promise<BanInfo | null> {
  if (!ip) return null
  const [row] = await db
    .select({
      id: bannedIp.id,
...
export async function getEffectiveBan(userId: string | null): Promise<BanInfo | null> {
  if (userId) {
