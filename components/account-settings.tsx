...
export function AccountSettings() {
  ...
  return (
    <div className="mt-10 space-y-10">
        ...
        <form onSubmit={handleChangePassword} className="space-y-4">
          ...
          <div className="flex justify-end">
            <Button type="submit" disabled={savingPw} className="gap-2">
              {savingPw ? <Loader2 className="size-4 animate-spin" aria-hidden /> : null}
              Update password
            </Button>
          </div>
        </form>
      ...
      {/* Danger zone */}
      <section className="space-y-4 rounded-xl border border-destructive/40 bg-destructive/5 p-5">
        <div className="flex items-center gap-2">
          <Trash2 className="size-4 text-destructive" aria-hidden />
          <h2 className="text-lg font-semibold tracking-tight text-destructive">Delete account</h2>
        </div>
        <p className="text-sm leading-relaxed text-muted-foreground">
          Permanently delete your account, profile, posts, and messages. This cannot be undone.
        </p>
        <Dialog>
