...
export function NotificationSettings() {
  ...
  function setMaster(patch: Partial<Pick<NotificationPreferences, "soundEnabled" | "volume">>) {
    update({ ...prefs, ...patch })
  }

  function setCategory(key: NotificationCategory, patch: Partial<{ popup: boolean; sound: boolean }>) {
    update({
      ...prefs,
      categories: { ...prefs.categories, [key]: { ...prefs.categories[key], ...patch } },
    })
  }
      ...
      <section className="space-y-4">
        ...
        <div className="flex items-center justify-between gap-4 rounded-xl border border-border bg-card p-4">
          <div className="min-w-0">
            ...
            <p className="text-sm text-muted-foreground">A short chime plays for enabled events</p>
          </div>
          <Switch
        ...
        <div className="rounded-xl border border-border bg-card p-4">
          <div className="mb-3 flex items-center justify-between">
            ...
            <span className="flex items-center gap-1.5 text-sm tabular-nums text-muted-foreground">
              {volumePct === 0 ? <VolumeX className="size-4" aria-hidden /> : <Volume2 className="size-4" aria-hidden />}
              {volumePct}%
            </span>
          </div>
          <div className="flex items-center gap-3">
            <Slider
              ...
              onValueChange={(v) => setMaster({ volume: Array.isArray(v) ? v[0] : v })}
