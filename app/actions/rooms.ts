...
export async function createRoom(name: string): Promise<{ chatId: string }> {
  ...
  await db.insert(chatParticipant).values({ id: newId("cp"), chatId, userId: me.id })
  return { chatId }
}
...
export async function joinRoom(chatId: string): Promise<{ chatId: string }> {
  ...
  const [c] = await db
    .select()
    .from(chat)
    .where(and(eq(chat.id, chatId), eq(chat.type, "GROUP"), isNull(chat.endedAt)))
    .limit(1)
...
export async function leaveRoom(chatId: string) {
  ...
  await db
    ...
    .set({ leftAt: new Date() })
    .where(
      and(eq(chatParticipant.chatId, chatId), eq(chatParticipant.userId, me.id), isNull(chatParticipant.leftAt)),
    )
  return { ok: true }
}
...
export async function myRooms(): Promise<RoomSummary[]> {
  ...
  const myMemberships = db
    ...
    .from(chatParticipant)
    .where(and(eq(chatParticipant.userId, me.id), isNull(chatParticipant.leftAt)))
  ...
  const rows = await db
    ...
    .from(chat)
    .leftJoin(chatParticipant, eq(chatParticipant.chatId, chat.id))
    .where(and(eq(chat.type, "GROUP"), isNull(chat.endedAt), sql`${chat.id} in ${myMemberships}`))
    .groupBy(chat.id, chat.name, chat.createdAt)
    .orderBy(desc(chat.createdAt))
