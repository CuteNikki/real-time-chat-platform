...
async function loadModerationTarget(targetUserId: string, action: "ban" | "delete") {
  const actorRole = await requireRole(action === "delete" ? "ADMIN" : "MODERATOR")
  const me = await getCurrentUser()
...
export async function banUser(
  ...
  const expiresAt =
    opts.durationDays != null ? new Date(Date.now() + opts.durationDays * 24 * 60 * 60 * 1000) : null

  // Lift any currently-active ban so there is only ever one active row.
  await db
    .update(ban)
    .set({ liftedAt: new Date(), liftedById: me.id, liftReason: "Superseded by a new ban" })
  ...
  const [lastSession] = await db
    ...
    .from(session)
    .where(and(eq(session.userId, targetUserId), sql`${session.ipAddress} is not null`))
  ...
  await db.update(user).set({ isBanned: true, banExpiresAt: expiresAt }).where(eq(user.id, targetUserId))
  ...
  if (opts.banIp && capturedIp) {
    ...
    await db
      ...
      .where(and(eq(bannedIp.ipAddress, capturedIp), isNull(bannedIp.liftedAt)))
...
export async function unbanUser(targetUserId: string, reason = "Ban lifted by moderator") {
  ...
  await db.update(user).set({ isBanned: false, banExpiresAt: null }).where(eq(user.id, targetUserId))
...
export async function deleteUser(targetUserId: string) {
  ...
  const ownPosts = await db.select({ id: post.id }).from(post).where(eq(post.userId, targetUserId))
  ...
  await db.delete(invite).where(or(eq(invite.senderId, targetUserId), eq(invite.receiverId, targetUserId)))
  await db.delete(message).where(eq(message.senderId, targetUserId))
  await db.delete(chatParticipant).where(eq(chatParticipant.userId, targetUserId))
  await db
    .delete(notification)
    .where(or(eq(notification.userId, targetUserId), eq(notification.actorId, targetUserId)))
  await db.delete(randomQueue).where(eq(randomQueue.userId, targetUserId))
  await db
    .delete(report)
    .where(or(eq(report.reporterId, targetUserId), eq(report.reportedUserId, targetUserId)))
...
export async function getBanHistory(targetUserId: string): Promise<BanHistoryEntry[]> {
  await requireRole("MODERATOR")
  ...
  return [...account, ...ip].sort((a, b) => (a.createdAt < b.createdAt ? 1 : -1))
}
